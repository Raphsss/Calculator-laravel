...
@section('title', 'Calcular média')

@section('content')
    <main class="flex-1 p-10 text-gray-900 flex flex-col justify-center items-center">
        <h1 class="text-3xl font-semibold mb-6 text-center">Calcular média</h1>

        <section class="max-w-sm mx-auto bg-white p-6 rounded-lg shadow-md">
            <form action="{{ route('calculateAverage') }}" method="post" class="space-y-4">
                @csrf
                ...
                <div>
                    <label for="num03" class="block text-sm font-medium text-gray-700">Insira o terceiro valor:</label>
                    <input type="number" name="num03" id="num03" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500">
                </div>
